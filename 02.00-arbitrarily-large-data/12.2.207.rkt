...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 12.2.207) (read-case-sensitive #t) (teachpacks ((lib "batch-io.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "batch-io.rkt" "teachpack" "2htdp")) #f)))
...
(define LASSOC1 (cons (cons "Track ID" (cons 86 '())) '())) ; (list (list "Track ID" 86))
(define LASSOC2 (cons (cons "Genre" (cons "Rock" '())) LASSOC1)) ; (list (list "Genre" "Rock") (list "Track ID" 86))
(define LASSOC3 (cons (cons "Total Time" (cons 284865 '())) LASSOC2))

(define LASSOC4 (cons (cons "Total Time" (cons 284865 '())) '()))
(define LASSOC5 (cons (cons "Has been played" (cons #true '())) '()))
(define LASSOC6 (cons (cons "Music type" (cons #false '())) '()))
; 
...
(define LLISTS1 (cons LASSOC1 (cons LASSOC2 (cons LASSOC3 (cons LASSOC4 '())))))
(define LLISTS2 (cons LASSOC1 '()))
(define LLISTS3 (cons LASSOC1 (cons LASSOC2 '())))
(define LLISTS4 (cons LASSOC1 (cons LASSOC2 (cons LASSOC3 '()))))
(define LLISTS5 (cons LASSOC1 (cons LASSOC2 (cons LASSOC3 (cons LASSOC5 '())))))
(define LLISTS6 (cons LASSOC1 (cons LASSOC5 (cons LASSOC6 '()))))
...
(check-expect (find-association "" '() "empty") "empty")
(check-expect (find-association "Track ID" LASSOC1 "empty") (list "Track ID" 86))
(check-expect (find-association "Genre" LASSOC1 "empty") "empty")
(check-expect (find-association "Genre" LASSOC2 "empty") (list "Genre" "Rock"))

; (define (find-association key loa default) (list "name" "value")) ;stub
(define (find-association key loa default)
  (cond
    [(empty? loa) default]
    [else
     (if (string=? key (first (first loa)))
         (first loa)
         (find-association key (rest loa) default))]))

; LLists -> Number
; produces the total amount of play time
(check-expect (total-time/list '()) 0)
(check-expect (total-time/list LLISTS2) 0)
(check-expect (total-time/list LLISTS3) 0)
(check-expect (total-time/list LLISTS4) 284865)

; (define (total-time/list llst) 0) ;stub
(define (total-time/list llst)
  (cond [(empty? llst) 0]
        [else
         (+ (get-value (find-association "Total Time" (first llst) (list "empty" 0)))
            (total-time/list (rest llst)))]))

; Association -> Any
; get value from the key-value pair
(check-expect (get-value (list "track id" 20)) 20)
(check-expect (get-value (list "album" "album name")) "album name")

; (define (get-value assoc) 0) ;stub
(define (get-value assoc)
  (list-ref assoc 1))

; Association -> Any
; get value from the key-value pair
(check-expect (get-attr (list "track id" 20)) "track id")
(check-expect (get-attr (list "album" "album name")) "album")

; (define (get-value assoc) 0) ;stub
(define (get-attr assoc)
  (list-ref assoc 0))

; LLists -> List-of-strgins
(check-expect (boolean-attributes '()) '())
(check-expect
 (member "Has been played" (boolean-attributes LLISTS5))
 #true)
(check-expect
 (and
  (member "Has been played" (boolean-attributes LLISTS6))
  (member "Music type" (boolean-attributes LLISTS6)))
 #true)

; (define (boolean-attributes llst) '()) ;stub
(define (boolean-attributes llst)
  (cond
    [(empty? llst) '()]
    [else
     (append (get-bool-values (first llst))
           (boolean-attributes (rest llst)))]))

; LASSOC -> List-of-strings
(check-expect (get-bool-values '()) '())
(check-expect (get-bool-values LASSOC5) (list "Has been played"))
(check-expect (get-bool-values LASSOC6) (list "Music type"))


; (define (get-bool-values las) '()) ;stub
(define (get-bool-values las)
  (cond
    [(empty? las) '()]
    [else
     (if (boolean? (get-value (first las)))
         (cons (get-attr (first las))
               (get-bool-values (rest las)))
         (get-bool-values (rest las)))]))

; 208 - solution
; (create-set (boolean-attributes list-tracks))