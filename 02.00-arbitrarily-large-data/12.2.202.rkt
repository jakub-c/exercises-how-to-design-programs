...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 12.2.202) (read-case-sensitive #t) (teachpacks ((lib "batch-io.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "batch-io.rkt" "teachpack" "2htdp")) #f)))
...
(check-expect (select-album "Scientific Wax 027" LTRACKS) (list "The Meaning of Life"))
  ...
  (cond
    ...
    [else
     (if (string=? title (track-album (first lot)))
          (cons (track-name (first lot)) (select-album title (rest lot)))
          (select-album title (rest lot)))]))

; String Date LTracks -> LTracks
;(define (select-album-date
